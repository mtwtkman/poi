...
type DiscribedTrashData = (LocalTime, U.UUID, PoiTestData)
...
flattenTrashData :: [PackedTrashData] -> [DiscribedTrashData]
flattenTrashData = foldr (\(t, ds) acc -> map (\(fid, d) -> (t, fid, d)) ds <> acc) []
