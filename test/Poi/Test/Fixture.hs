...
createTrashedData :: LocalTime -> U.UUID -> PoiTestData -> Trash
createTrashedData t fid (PoiTestData{poiTestDataParentNode = p, poiTestDataLeaf = f}) = Trash f p fid t

poiTesteDataToTrashBatch :: PackedTrashData -> [Trash]
poiTesteDataToTrashBatch (t, signed) = fmap (\(fid, PoiTestData{poiTestDataParentNode = p, poiTestDataLeaf = f}) -> Trash f p fid t) signed
